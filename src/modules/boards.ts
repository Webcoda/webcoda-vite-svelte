...
export default (el, $el, opts) => {
	...
	let enableScrollToActive = !opts.disableAutoScrollOnFirstClick;

	const attachListener = () => {
		$el.find(opts.panelSelector).off('click').on('click', ev => {
			ev.preventDefault()
			...
			if (!isActive) {
				...
				$theCols.each((i, el) => {
					...
					Object.keys(opts.cols).forEach(key => {
						if ($el.is(`${opts.detailSelector}`) && $el.hasClass(`${key}:block`) && !temp.includes(key)) {
			...
			const onCollapseShownScroll = ev => {
				const $el = $(ev.currentTarget)
				if ($el.closest(opts.detailSelector).is(':visible') && enableScrollToActive) {
					console.log('scrollToActiveProfile')
					scrollToActiveProfile()
				}
				enableScrollToActive = true;
			}

			const $theCollapse = $el
				.find(`${opts.detailSelector}.active`)
				.find('.collapse')
				.one('shown.bs.collapse', debounce(onCollapseShownScroll, 100))
			...
			if (!isActive) {
				...
				if (prevSelectedColIndex !== selectedColIndex && prevSelectedColIndex !== '') {
					...
					if (Math.floor(parseSelectedColIndex / cols) !== Math.floor(parsePrevSelectedColIndex / cols)) {
						$el.find(`${opts.detailSelector}:not(.active)`)
							.find('.collapse')
			...
			if (prevSelectedColIndex !== selectedColIndex) {
				$el.find(opts.panelSelector)
					.not($this)
					.removeClass('active')
			}
		})

		$el.find(opts.detailCloseSelector).off('click').on('click', ev => {
			const $this = $(ev.currentTarget)
