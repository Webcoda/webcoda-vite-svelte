...
import '../styles/modules/socialstream.scss'
...
export default class SocialStream {
	...
	getFilterItemHtml(key, filterKey, label, icon, attr, additionalClass = '') {
		return `<div class="pb-1 mr-6 relative o-socialstream__filter-item ${additionalClass}" role='presentation'>
			<button title="Show ${label || key} feeds" aria-label="Show ${label ||
			key} feeds" type="button" class="relative w-full flex justify-center items-center text-inherit o-socialstream__filter-btn o-socialstream__filter-btn--${key}" data-filter="${filterKey}" ${attr}>
				${
					icon
				...
				<span class="b-fsbtnlarge uppercase align-middle hidden-xs o-socialstream__filter-btn-text" aria-hidden="true">${label ||
					key}</span>
			</button>
	...
	getFacebookHtml(items, orderNumber) {
		return (
			...
			items
				.map(item => {
					return `<div class="filtr-item col-12 sm-col-6 md-col-4 lg-col-3 pt-2 pb-2 o-socialstream__col o-socialstream__col--facebook" data-category="${orderNumber +
						1}">
	...
	getInstagramHtml(items, orderNumber) {
		...
		return items
			? items
					.map(item => {
						return `<div class="filtr-item col-12 sm-col-6 md-col-4 lg-col-3 pt-2 pb-2 o-socialstream__col o-socialstream__col--instagram" data-category="${orderNumber +
							1}">
				<div class="relative">
	...
	getTwitterHtml(items, orderNumber) {
		return items
			? items
					.map(item => {
						return `<div class="filtr-item col-12 sm-col-6 md-col-4 lg-col-3 pt-2 pb-2 o-socialstream__col o-socialstream__col--twitter" data-category="${orderNumber +
							1}">
	...
	getYoutubeHtml(items, orderNumber) {
		...
		return items
			? items
					.map(item => {
						return `<div class="filtr-item col-12 sm-col-6 md-col-4 lg-col-3 pt-2 pb-2 o-socialstream__col o-socialstream__col--youtube" data-category="${orderNumber +
							1}">
				<div class="relative">
